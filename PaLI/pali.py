...
from transformers import MT5ForConditionalGeneration, ViTModel, MT5Config, RobertaModel, AutoTokenizer
...
class PaLI_PhoBERT(nn.Module):
    def __init__(
        ...
        self.mt5.shared = phobert.embeddings

        print(f"Original input embedding shape: {original_input_embeddings.weight.shape}")
        print(f"New input embedding shape: {self.mt5.get_input_embeddings().word_embeddings.weight.shape}")
        ...
        assert self.mt5.get_input_embeddings().word_embeddings.weight.shape[0] == new_vocab_size ,"Input embedding mismatch"
        ...
        with torch.no_grad():
            self.mt5.get_input_embeddings().word_embeddings.weight.data = \
                phobert.embeddings.word_embeddings.weight.data.clone()
            self.mt5.get_input_embeddings().position_embeddings.weight.data =\
                phobert.embeddings.position_embeddings.weight.data.clone()
            self.mt5.get_input_embeddings().token_type_embeddings.weight.data = \
                phobert.embeddings.token_type_embeddings.weight.data.clone()
        ...
        if self.mt5.get_input_embeddings().word_embeddings is not self.mt5.get_output_embeddings():
            print("Input and output embeddings are not tied. Transplanting output head weights separately.")
            with torch.no_grad():
                self.mt5.get_output_embeddings().weight.data = phobert.embeddings.word_embeddings.weight.data.clone()
        else:
