...
class ViVQADataset(BaseDataset):
    ...
    def make_dataset_index(cls, data_path, tokenizer, annotation_data_path):
        ...
        # For PhoBERT
        with open(os.path.join(annotation_data_path, "train_segmented.json"), "r") as fp:
            train_vivqa = json.load(fp)["annotations"]
        with open(os.path.join(annotation_data_path, "test_segmented.json"), "r") as fp:
            test_vivqa = json.load(fp)["annotations"]
        ...
        for split, annots in zip(
            ...
            # _annot = annotations[split]
            for q in annots:
                all_major_answers.append(q["answer"][0])
...
class OpenViVQADataset(BaseDataset):
    ...
    @classmethod
    def make_dataset_index(cls, data_path, tokenizer, annotation_data_path):
        with open(os.path.join(annotation_data_path, "openvivqa_test_v2.json"), "r") as fp:
            annotations_test = json.load(fp)["annotations"]
        with open(os.path.join(annotation_data_path, "openvivqa_dev_v2_copy.json"), "r") as fp:
            annotations_val = json.load(fp)["annotations"]
        with open(os.path.join(annotation_data_path, "openvivqa_dev_v2.json"), "r") as fp:
            annotations_test_dev = json.load(fp)["annotations"]
        with open(os.path.join(annotation_data_path, "openvivqa_train_v2.json"), "r") as fp:
        ...
        for split, questions in zip(
            ["train", "val", "test", "test-dev"],
            [annotations_train, annotations_val, annotations_test, annotations_test_dev],
        ):
            _annot = defaultdict(dict)
            for q in questions:
                question_text = q["question"]
                ...
                assert q["id"] not in _annot[q["image_id"]]
                _annot[q["image_id"]][q["id"]] = {
                    "question": question_text, 
                    "token_ids": token_ids, 
                }
        ...
        for split, annots in zip(
            ...
            for q in annots:
                all_major_answers.append(q["answers"][0]) # Get most frequent 
        
        all_major_answers = [normalize_word(word) for word in all_major_answers]
        counter = {k: v for k, v in Counter(all_major_answers).items() if v >= 0} # Unlike VQAv2, OpenViVQA only 1 answers in json
        ans2label = {k: i for i, k in enumerate(counter.keys())}
        label2ans = list(counter.keys())
        ...
        for split, annots in zip(
            ...
            for q in annots:
                ...
                assert "labels" not in _annot[q["image_id"]][q["id"]]
                assert "question" in _annot[q["image_id"]][q["id"]]
                _annot[q["image_id"]][q["id"]]["labels"] = labels
                _annot[q["image_id"]][q["id"]]["scores"] = scores
        ...
        for split in ["train", "val", "test", "test-dev"]:
            ...
            # paths = list(glob.glob(f"{data_path}/{split_name}/*.jpg"))
            paths = list(glob.glob(f"{data_path}/images/*.jpg"))
            random.shuffle(paths)
            annot_paths = [path for path in paths \
                if int(path.split("/")[-1][:-4]) in annot]
            ...
            _write_data_into_jsonl(items=items, jsonl_file=os.path.join(data_path, "openvivqa.%s.jsonl" % split))

        # Following ViLT, we use 1000 images of the original val set as the final val set (?: Recheck for OpenViVQA)    
        val_image2items = defaultdict(list)
        for item in split2items["val"]:
            val_image2items[item["image_path"]].append(item)
        ...
        _write_data_into_jsonl(items=trainable_val, jsonl_file=os.path.join(data_path, "openvivqa.trainable_val.jsonl"))
        _write_data_into_jsonl(items=rest_val, jsonl_file=os.path.join(data_path, "openvivqa.rest_val.jsonl"))
