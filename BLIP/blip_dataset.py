...
from transformers import BlipProcessor, PhobertTokenizer
...
class ViVQABlibProcessor(BlipProcessor):
    tokenizer_class = ("BertTokenizer", "BertTokenizerFast", "PhobertTokenizer")

    def __init__(self, image_processor, tokenizer, **kwargs):
        super().__init__(image_processor, tokenizer, **kwargs)

class ViVQABlipDataset(torch.utils.data.Dataset):
    def __init__(self,
                 args,
        ...
        self.processor = BlipProcessor.from_pretrained(blip_model_id)
...
def create_blip_dataloader(dataset, is_train, batch_size, num_workers, pin_mem, dist_eval=False):
    if is_train:
...
def create_blip_dataset_by_split(args, split, is_train=True, phobert_tokenizer=None):
    en_json_suffix = 'en'
    ...
    data_loader = create_blip_dataloader(
...
def create_blip_datasets(args, is_eval=False, phobert_tokenizer=None):
    if is_eval:
        return create_blip_dataset_by_split(args, split="test", is_train=False, phobert_tokenizer=phobert_tokenizer)
    else:
        dataset_train, data_loader_train =create_blip_dataset_by_split(
            args, split="train", is_train=True, phobert_tokenizer=phobert_tokenizer
        )
        dataset_val, data_loader_val = create_blip_dataset_by_split(
