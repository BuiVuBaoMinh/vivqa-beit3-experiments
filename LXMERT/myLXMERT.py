...
from transformers import (
    AutoTokenizer, 
    LxmertForQuestionAnswering, LxmertPreTrainedModel, LxmertModel
)
from transformers.modeling_outputs import SequenceClassifierOutput

from glossary import segment_normalize

class LxmertForVqaClassification(LxmertPreTrainedModel):
    def __init__(self, config, num_labels, answer2label_path):
        ...
        print(f"LxmertForVqaClassification: num_labels = {self.num_labels}")
        
        # Use the base Lxmert model
        self.blip = LxmertModel(config)
